...
class ReceipDetail
{
    ...
    public function getReceiptsToday()
    {
        ...
        $this->db->query("SELECT 
                        ...
                        LEFT JOIN customers
                        ON receipt_detail.customer_id = customers.id
                        WHERE DATE(receipt_detail.updated_at) = '".$date."'
                        GROUP BY receipt_detail.receipt_id
                        ORDER BY receipt_detail.receipt_id DESC");
        return $this->db->resultSet();
    }
    ...
    public function getReceiptDetail($receipt_id)
    {
        $this->db->query('SELECT 
                        ...
                        ON receipt_detail.receipt_id = receipts.receipt_id
                        INNER JOIN stocks
                        ON receipt_detail.stock_id = stocks.stocks_shortcut_id
                        LEFT JOIN customers
                        ON receipt_detail.customer_id = customers.id
                        WHERE receipt_detail.receipt_id = :receipt_id
                        ORDER BY receipt_detail.disp_sort ASC');
        //Bind Values
    ...
    {
        $this->db->query("INSERT INTO receipt_detail (receipt_id, stock_id, customer_id, qty, customer_price, total, disp_sort, created_at, updated_at) VALUES (:receipt_id, :stock_id, :customer_id, :qty, :customer_price, :total, :disp_sort, :created_at ,:updated_at)");
        //Bind Values
    ...
    {
        $this->db->query("SELECT CASE WHEN MAX(receipt_id) IS NULL THEN 1 ELSE MAX(receipt_id) + 1 end as receipt_id FROM receipt_detail");
